...
func _ready() -> void:
	...
	if Globals.player2_choice == "rock":
	...
	if Globals.player2_choice == "rock" and Globals.player1_choice == "paper":
		winner = "player1"
	...
	if winner == "player1":
		...
		title.visible = false
	Globals.rounds_played += 1
	await get_tree().create_timer(3).timeout
	if Globals.rounds_played < Globals.no_of_rounds:
		TransitionScreen.transition()
		await TransitionScreen.on_transition_finished
		get_tree().change_scene_to_file("res://main game screens/choose_item_screen.tscn")
	elif Globals.rounds_played == Globals.no_of_rounds:
		TransitionScreen.transition()
		await TransitionScreen.on_transition_finished
